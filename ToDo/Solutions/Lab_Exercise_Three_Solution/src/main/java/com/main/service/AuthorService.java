...
public class AuthorService  {
    ...
    public Author createAuthor(Author a) {
        //POST must not accept a client-supplied ID
        if (a.getAuthorID() != 0) {
            throw new ResponseStatusException(
                    HttpStatus.UNPROCESSABLE_ENTITY,
                    "authorID must not be supplied when creating an author"
            );
        }
    ...
    public Author updateAuthor(Author a) {
        //PUT must specify the target to update
        long id = a.getAuthorID();
        if (id == 0) {
            throw new ResponseStatusException(
                    HttpStatus.UNPROCESSABLE_ENTITY,
                    "authorID is required when updating an author"
            );
        }

        //PUT must not create a new record
